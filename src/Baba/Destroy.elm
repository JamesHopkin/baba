...
doMutualDestroys cell = 
    let
        -- sink!
        sinkObjects = List.filter (Cell.objectIs Types.Sink) cell.contents
        numSinkObjects = List.length sinkObjects
    in
    if numSinkObjects > 0 && numSinkObjects < List.length cell.contents then
        Just Cell.emptyCell
        ...
        if and_ (Cell.cellHas Types.Open) (Cell.cellHas Types.Shut) cell then
            Just <| Cell.makeCell <|
                List.filter (\obj -> not (Cell.objectIsAny [Types.Open, Types.Shut] obj)) cell.contents
...
doUnilateralDestroys cell =
    ...
    let
        filterOut =
            (if List.length cell.contents > 1 then [Types.Weak] else [])
        ...
        filtered = List.filter filter cell.contents
    in
    -- hot/melt
    if List.length filtered /= List.length cell.contents then
        Just { contents = filtered, viewHints = Nothing }
            ...
            case LinkedGrid.at x y grid of
                Just loc ->
                    let
                        ...
                        combinedFlags = List.foldr orObjFlags 0 contents.contents
