...
flipCellDirection axis =
    (LinkedGrid.axisGet axis).contents
        |> List.map flipDir
        |> (\cell -> LinkedGrid.axisSet (makeCell cell) axis)
        |> LinkedGrid.flipAxis
            ...
            case LinkedGrid.axisForward 1 fromAxis of
                Just axis ->
                    let
                        ...
                        cell = LinkedGrid.axisGet axis
                        pushIds = List.filter (objectIs Types.Push) cell.contents
                    ...
                    else if cellHasAny [Types.Pull, Types.Stop] cell

                        -- special case to allow keys to open doors
                        && not (cellHas Types.Shut cell && (cellHas Types.Open <| LinkedGrid.axisGet fromAxis)) then
            ...
            case LinkedGrid.axisForward -1 fromAxis of
                Just axis ->
                    let
                        ...
                        pullIds =
                            (LinkedGrid.axisGet axis).contents
                            |> List.filter (objectIs Types.Pull)
                            |> List.map getObjectId
                    in
            ...
            let
                ...
                addIfMove loc acc =
                    let
                        ...
                        movingContent = 
                            (LinkedGrid.getContents loc).contents
