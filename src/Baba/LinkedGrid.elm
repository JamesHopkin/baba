...
type Row el = Row (LinkedGrid el) Int (Array el)

getDimensions (LinkedGrid _ width height _) = ( width, height )
...
foldRows : (Row el -> a -> a) -> a -> LinkedGrid el -> a
foldRows f initial grid =
    case grid of
        LinkedGrid _ _ height rows ->
            let
                foldFunc row ( y, acc ) = ( y - 1, f (Row grid y row) acc )
            in

            Array.foldr foldFunc ( height - 1, initial ) rows
                |> Tuple.second

foldRowLocations : (Location el -> a -> a) -> a -> Row el -> a
foldRowLocations f initial row =
    case row of
        Row grid y arr ->
            foldRange (\x -> f (Location grid x y)) initial (Array.length arr)
